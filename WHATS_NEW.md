...
- **Project overlays, for every agent.** Drop a `.mp-sentinel/skill-overlay.md` (or set `createSkills.overlayFile`) and it is copied verbatim into every generated skill and rule file as `## Project Overlay (authoritative)`, above the generated rules. It feeds the generation-config hash, so editing it makes `--check` report stale. Overlay typography is normalised to ASCII and the overlay is exempt from the unknown-path check, so ordinary prose does not trip a gate meant for generated text. This replaces post-processing scripts that patched one agent's `SKILL.md` and were undone by the next `--force`.
