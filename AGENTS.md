...
- If the source index does not contain a project name (no detected manifest has a `"name"` field), the command fails with exit code `2` rather than generating files under the generic `"project"` name.
...
- [ ] `npm run dogfood` — full local workflow validates end-to-end (indexing, index queries, create-skills, explain-agents, explain-context unavailable + available, doctor, agent:skills:check).
